...
function random_sampling(index_pool, logit_pool, n_query)
...
function entropy_sampling(index_pool, logit_pool, n_query)
    entr_pool = entropy(softmax(logit_pool))
    perm = sortperm(entr_pool, rev=true)
    return index_pool[perm[1:n_query]]
end

function oracle(index_query, logit_query, round)
    # Oracle labels the whole query.
    return index_query
end

function human_labeller(index_query, logit_query, round, y, file)
    entr_query = entropy(softmax(logit_query))
    h5open(file, "r+") do datafile
        write(datafile, @sprintf("index_query_%d", round), index_query)
        write(datafile, @sprintf("logit_query_%d", round), logit_query)
...
function simulate_al(query, labeller, model, X, y, X_test, y_test;
    ...
    for round in 1:n_round
        ...
        logit = forward(model_gpu, gpu(X_gpu))
        # Query (might be only preselection).
        index_query = query(index_pool, logit[:, index_pool], n_query)
        # Oracle (might be an inperfect human labeller that might not label the whole query).
        index_label = labeller(index_query, logit[:, index_query], round)
